...
from IrisScanning.utils.math_utils import is_between_parabolas_coords
from IrisScanning.utils.image_utils import valid_pixel
from IrisScanning.utils.error_utils import SUCCESS
#--------------------------------------------------------------------------------
...
def normalize_iris(img, angular_resolution, radial_resolution, pupil_center, pupil_radius, iris_center, iris_radius, upper_eyelid, lower_eyelid):
    ...
    for col in range(angles):
        ...
        for row in range(radii):
            ...
            pixel_value = int(img).item(y, x)    # indexed first by rows, then by columns
